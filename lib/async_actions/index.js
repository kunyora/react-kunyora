...
var draw = function draw(timeFraction) {
  return Math.pow(timeFraction, 2);
};

var startProgressCount = exports.startProgressCount = function startProgressCount(name) {
    ...
    try {
      ...
      var start = window.performance.now(),
        ...
        if (progress <= 0.8) {
          ...
          store.dispatch(types.SET_PAGE_DOWNLOAD_PROGRESS, _extends({}, overallState, _obj));
 ...
 * @param {String} name
 */
var completeProgressCount = exports.completeProgressCount = function completeProgressCount(name) {
  return function (store) {
    try {
      var start = window.performance.now(),
          duration = 1000,
        ...
        if (movement >= progress && movement < 1) {
          ...
          var _obj2 = {};
          _obj2[name] = 0;
          store.dispatch(types.SET_PAGE_DOWNLOAD_PROGRESS, _extends({}, overallState, _obj2));
          progress = 0;
          animation = null;
        } else {
          requestAnimationFrame(animate);
